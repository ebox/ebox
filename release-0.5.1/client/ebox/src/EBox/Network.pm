...
# returns a reference to a list of all external network interfaces
sub ExternalIfaces
{
	my $self = shift;
...
# returns a reference to a list of all internal network interfaces
sub InternalIfaces
{
	my $self = shift;
	my @ifaces = @{$self->ifaces};
...
# returns true if the given network interfaces (virtual or real) exists and is
# external
...
# returns
#	- a ref to an array with the names of all real interfaces, including 
#	  vlan interfaces
sub ifaces
{
	my $self = shift;
...
# 	- the name of a network interface
# returns
#	- an ref to an array with hashes with keys containing name, ip, and 
#	  netmask. 
...
# returns
# 	- an array with the names of all (real and virtual) interfaces. This
...
# returns true if the given vlan ID exists
...
# returns a reference to an array. The elements of the array are hashes with
# information about every vlan that exists on the given trunk interface. The
# hashes contain theses fields:
# 	+ id (vlan ID)
# 	+ name (user given description for the vlan)
# 	+ interface (the name of the trunk interface)
