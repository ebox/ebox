...
wget http://archive.ubuntu.com/ubuntu/indices/override.$VERSION.{extra.main,main,main.debian-installer,restricted,restricted.debian-installer}
...
for TEMPLATE in $CONF_FILE_TEMPLATES; do
   ...
   sed -e s:INDICES:$INDICES_DIR: -e s:ARCHIVE_DIR:$CD_BUILD_DIR: < $TEMPLATE  > $CONF_FILE
done
